...
from lib.loader import my_cv2_imread_rgb, my_open
...
class OceanDataset(Dataset):
    def __init__(self, cfg):
        ...
        '''2020.12.18 mark the DATA_DIR'''
        self.DATA_DIR = cfg.OCEAN.DATASET.DATA_DIR
        self.grids()
    ...
    def __getitem__(self, index):
        ...
        template_image = my_cv2_imread_rgb(template[0], self.DATA_DIR, dataset.lower_name)
        search_image = my_cv2_imread_rgb(search[0], self.DATA_DIR, dataset.lower_name)
...
class subData(object):
    ...
    def __init__(self, cfg, data_name, start):
        ...
        self.start = start
        name_dict = {'YTB': 'y2b', 'VID': 'vid511', 'COCO': 'coco', 'DET': 'det', 'GOT10K': 'got10k'}
        json_dict = {'YTB': 'train.json', 'VID': 'train.json', 'COCO': 'train2017.json', 'DET': 'train.json', 'GOT10K': 'all.json'}
        lower_name = name_dict[data_name]
        json_name = json_dict[data_name]
        self.lower_name, self.json_name = lower_name, json_name
        ...
        prefix = cfg.OCEAN.DATASET.DATA_DIR
        self.root = os.path.join(prefix, '%s/crop511'%lower_name)
        self.annotation = os.path.join(prefix, '%s/%s'%(lower_name, json_name))

        # with open(info.ANNOTATION) as fin:
        # with my_open(self.annotation) as fin:
        with my_open(self.annotation, prefix=prefix, lower_name=lower_name) as fin:
